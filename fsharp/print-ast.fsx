...
let parse isFsi code =
    Parse.parseFile isFsi (SourceText.ofString code) [] |> fst

let parseImpl = parse false
...
let todoExpr =
    SingleTextNode("todo!", zeroRange) |> Constant.FromText |> Expr.Constant
...
let rec map (value: obj) : Expr =
    ...
    if FSharpType.IsUnion t then
        ...
        sprintf "%s.%A" t.Name value |> mkConstExpr
...
and mapList (value: obj) =
    ...
    | _ -> failwithf "Excepted %A to implement IEnumerable" value
